...
def test_empty_client(rows_to_select, remove_data=True):
    def test_case(self):
        try:
            ...
            self.log('Selecting client to add services')
            ...
            self.is_equal(selected_rows, open_rows, test_name, 'CHECK IF CORRECT ROWS ARE DISPLAYED',
                          'CHECK IF CORRECT ROWS ARE DISPLAYED FAILED')
        except Exception:
            self.log('ERROR' + traceback.print_exc(file=sys.stdout))
...
def test_existing_client(rows_to_select, remove_data=True):
    def test_case(self):
        try:
            self.log('Adding a service to a client')
            ...
            self.is_equal(selected_services, recently_added_services,
                          test_name,
                          'CHECK IF CORRECT SERVICES ARE ADDED',
                          'CHECK IF CORRECT SERVICES ARE ADDED FAILED')
        except Exception:
            self.log('ERROR' + traceback.print_exc(file=sys.stdout))
